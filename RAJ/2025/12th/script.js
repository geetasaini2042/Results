...
function sendResultToTelegram(roll, btn, ad) {
  ...
  .then(response => {
    if (!response || response.ok !== true) {
      ...
      fetch(`https://sainipankaj12.serv00.net/Result/boardresult.php?tag=raj_12th_result&roll_no=${roll}&year=2025&wb_id=89&source=3&see`)
        .then(res => res.text())
        .then(html => {
  ...
  .catch((error) => {
    ...
    ad.innerHTML = `<span style="color: red; font-weight: bold;">Failed to send result. Please Wait...</span>`;
    window.location.href = `get.html?roll_no=${roll}&year=2025&wb_id=89`;
  resetButton(btn);
});
}
