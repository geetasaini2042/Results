...
function sendResultToTelegram(roll, btn, ad) {
  const pdfUrl = `https://sainipankaj12.serv00.net/Result/boardresult.php?tag=raj_10th_result&roll_no=${roll}&year=2025&wb_id=88&source=3&download`;
  ...
  .then(response => {
    if (!response || response.ok !== true) {
      ...
      fetch(`https://sainipankaj12.serv00.net/Result/boardresult.php?tag=raj_10th_result&roll_no=${roll}&year=2025&wb_id=88&source=3&see`)
        .then(res => res.text())
        .then(html => {
