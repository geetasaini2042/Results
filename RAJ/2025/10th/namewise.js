...
function sendResult(name, btn, ad) {
  ...
  const getUrl = `https://sainipankaj12.serv00.net/Result/Namewise/result.php?name=${encodeURIComponent(name)}&page=1&url=${encodeURIComponent(sourceUrl)}`;
  const targetUrl = `https://sainipankaj12.serv00.net/Result/Namewise/result.php?name=${encodeURIComponent(name)}&page=1&user_id=${user.id}&url=${encodeURIComponent(sourceUrl)}`;
